...
namespace PokerMuck
{
    public partial class HoldemCardsSelector : UserControl
    {
        ...
        {
            foreach (HoldemCardsButton b in this.Controls)
            {
                b.Selected = false;
            }

            foreach(String card in cards){
                ((HoldemCardsButton)this.Controls["btn" + card]).Selected = true;
            }
        }
        ...
        private void Populate()
        {
            ...
            for (int i = 0; i < faces.Count<CardFace>(); i++)
            {
                ...
                for (int j = i; j < faces.Count<CardFace>(); j++)
                {
                    ...
                    // Suited
                    HoldemCardsButton suitedButton = CreateCardButton(f1, f2, true);
                ...
                for (int j = i; j < faces.Count<CardFace>(); j++)
                {
                    ...
                    // Offsuit
                    HoldemCardsButton offsuitedButton = CreateCardButton(f1, f2, false);
        ...
        private HoldemCardsButton CreateCardButton(CardFace f1, CardFace f2, bool suited)
        {
            HoldemCardsButton button = new HoldemCardsButton();
